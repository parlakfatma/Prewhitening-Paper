...
theVars <- list(
  ...
  acquisition=c("LR", "RL"), 
  test=c("test", "retest"),
  nreg=c("ar0_HRF", "ar0_dHRF"), # c("ar0_HRF", "ar6_HRF", "ar0_dHRF", "ar6_dHRF")
  vox=seq(nVox)
)
  ...
  for (task in tasks) {
    ...
    for (ii in seq(nrow(iters))) {
      ...
      if ("const" %in% names(ac)) {
        ...
        agg[task, paste0("s", subject), acquisition, ifelse(test, "test", "retest"),,] <- do.call(rbind, lapply(ac, function(x){x[[meas]]}))
      }
    }
