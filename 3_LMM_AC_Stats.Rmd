...
# dir_project <- "G:/.shortcut-targets-by-id/1-9kU5O4-bU0AeEZKukSvz4jbvolppJGG/ddpham/Prewhitening-Paper" # Damon's personal
dir_project <- "/Volumes/GoogleDrive/My Drive/MEJIA_LAB_Fatma/Projects/Prewhitening_Paper/code" # Fatma's personal
...
for (ii in seq(length(iters))) {
  ...
  for (vv in seq(nVox)) {
    ...
    q <- lme4::lmer(value ~ -1 + task + acquisition + cHRF:task  + (-1 + cHRF:task | subject) + (-1 + task | subject), data = df_vv)
