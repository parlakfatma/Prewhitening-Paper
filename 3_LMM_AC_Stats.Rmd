...
dir_project <- "/Volumes/GoogleDrive/My Drive/MEJIA_LAB_Fatma/Projects/Prewhitening_Paper/code" # Fatma personal
...
for (ii in seq(length(iters))) {
  ...
  for (vv in seq(nVox)) {
    ...
    q <- lme4::lmer(value ~ -1 + task + acquisition + task:cHRF  + (-1 + cHRF:task | subject) , data = df_vv)
