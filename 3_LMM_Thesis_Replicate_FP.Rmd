...
for (meas in c("ar6c")) {
  ...
  # fixed_fx <- array(matrix(nrow=nVox, ncol = 5),2)  # (4 tasks + 1 acquisition) + 1 for the model
  fixed_fx <- array(NA, dim= c(nVox, ncol= 5, 4))
  dimnames(fixed_fx) <- list(1:nVox, 1:5, c("dHRF_pw", "HRF_pw", "dHRF_npw", "HRF_npw")) # the last dimension represents the prewhitening & nonprewhitening
  # var_cor <- matrix(nrow=nVox, ncol = 11) # (4 for diagonal + 3*4/2 for off-diagonals + 1 for residual + 1 for model
  var_cor <- array(NA, dim = c(nrow=nVox, ncol = 11, 4)) # (4 for diagonal + 3*4/2 for off-diagonals + 1 for residual + 1 for model
  dimnames(var_cor) <- list(1:nVox, 1:11, c("dHRF_pw", "HRF_pw", "dHRF_npw", "HRF_npw")) # the last dimension represents the prewhitening & nonprewhitening
  ...
  for (vv in seq(nVox)) {
    ...
    for(idf in 1:length(list_df)){
      df_vv <- list_df[[idf]]
      q <- lme4::lmer(value ~ -1 + task + acquisition + (-1+task|subject) , data = df_vv)
      fixed_fx[vv,,idf] <- c(fixef(q))
      var_cor[vv,,idf] <- c((as.data.frame(VarCorr(q))$sdcor)) # Std.Dev. + Corr + residual
  ...
  saveRDS(list(fixed_fx = fixed_fx, var_cor = var_cor), file.path(dir_lm, paste0(meas, "LMM_FP.rds")))
