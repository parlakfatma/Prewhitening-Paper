...
df_aci_local$pw_FWHM <- "localPW"

df_aci_global <- df_aci_all %>% filter(pw_FWHM == "Inf")
df_aci_global$pw_FWHM <- "globalPW"
df_aci <- rbind(df_aci_local, df_aci_global)


ggplot(data = df_aci, aes(x = pw_order, y = avgACI, color = pw_FWHM) )+
            geom_point() + facet_grid(~HRF) +
            theme_light()
ggsave("avgACI.png")

ggplot(data = df_aci, aes(x = pw_order, y = avgACI, color = pw_FWHM, shape = HRF) )+
     geom_point(size = 10) +
